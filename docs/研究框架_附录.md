...
- [附录 K RQ5 补充图](#附录-k-rq5-中的连续密度估计器-benchmark-补充图)
  - [K.1 Transport map density estimation for continuous EI and synergy](#k1-transport-map-density-estimation-for-continuous-ei-and-synergy)
  - [K.2 连续密度估计器 benchmark 补充图与说明](#k2-连续密度估计器-benchmark-补充图与说明)
- [附录 L RQ5 线性高斯补充实验](#附录-l-rq5-中的线性高斯最小机制与相关噪声补充结果)
  - [L.1 二维最小机制对照](#l1-二维最小机制对照)
  - [L.2 固定机制下的相关噪声扫描](#l2-固定机制下的相关噪声扫描)
- [附录 M 空气污染实验中的 MLP 构建与训练细节](#附录-m-空气污染实验中的-mlp-构建与训练细节)
  - [M.1 样本构造与标准化](#m1-样本构造与标准化)
  - [M.2 JointStationMLP 的结构](#m2-jointstationmlp-的结构)
  - [M.3 训练目标、早停与基线](#m3-训练目标早停与基线)
  - [M.4 与 transport-map 因果图的衔接](#m4-与-transport-map-因果图的衔接)
...
### K.1 Transport map density estimation for continuous EI and synergy

This appendix clarifies the transport-map estimator used in the continuous RQ5 notebooks. The purpose of the estimator is not to replace the local Jacobian--Covariance formula in the mechanism-level analysis, but to provide a sample-based distributional counterpart: given intervention samples from source variables and model-generated target samples, estimate the joint and marginal densities needed for mutual information, then form the same joint-minus-single synergy statistic used throughout the paper.

Let $\mathbf{z}\in\mathbb{R}^d$ denote the concatenated sample vector whose components may include source features, target variables, or both. A monotone triangular transport map is a differentiable map

$$
\mathbf{T}(\mathbf{z})
=
\bigl(T_1(z_1),T_2(z_1,z_2),\dots,T_d(z_1,\dots,z_d)\bigr),
\qquad
\partial_{z_k}T_k(z_1,\dots,z_k)>0,
\tag{K.1}
$$

that pushes the unknown target distribution $\mu$ to a simple reference distribution $\eta$, usually a standard Gaussian. The triangular structure corresponds to the Knothe--Rosenblatt rearrangement and makes the Jacobian determinant tractable, while monotonicity guarantees invertibility along each conditional coordinate. Under the pullback convention $\mathbf{T}_{\#}\mu=\eta$, the target density is evaluated by the change-of-variables identity

$$
\log p_{\mu}(\mathbf{z})
=
\log p_{\eta}\!\bigl(\mathbf{T}(\mathbf{z})\bigr)
+
\log \left|\det \nabla \mathbf{T}(\mathbf{z})\right|
=
\log p_{\eta}\!\bigl(\mathbf{T}(\mathbf{z})\bigr)
+
\sum_{k=1}^{d}\log \partial_{z_k}T_k(z_1,\dots,z_k).
\tag{K.2}
$$

Baptista, Marzouk, and Zahm show that monotone triangular maps form a principled representation of the Knothe--Rosenblatt map, can be learned from samples by maximum-likelihood-type objectives, and admit adaptive sparse semi-parametric parameterizations for joint and conditional density estimation [8]. The broader measure-transport literature emphasizes the same practical point: a deterministic map between a complex distribution and a simple reference distribution turns density evaluation, sampling, conditioning, and Gaussianization into operations on an explicit transformation [9]. These properties are precisely why transport maps are useful in our setting, where $EI$ and synergy require repeated estimates of high-dimensional joint and marginal densities under controlled interventions.

The implementation used in the notebooks is a deliberately conservative approximation to this general framework. For a sample matrix $\{\mathbf{z}^{(m)}\}_{m=1}^{M}$, it fits the affine lower-triangular map induced by the empirical mean $\widehat{\boldsymbol{\mu}}$ and Cholesky factor $\widehat{\mathbf{L}}$ of the covariance matrix:

$$
\mathbf{T}_{\mathrm{aff}}(\mathbf{z})
=
\widehat{\mathbf{L}}^{-1}(\mathbf{z}-\widehat{\boldsymbol{\mu}}),
\qquad
\log \widehat{p}_{\mathrm{tm}}(\mathbf{z})
=
-\frac{1}{2}\left[d\log(2\pi)+\|\mathbf{T}_{\mathrm{aff}}(\mathbf{z})\|_2^2\right]
-\log|\det \widehat{\mathbf{L}}|.
\tag{K.3}
$$

Thus the present estimator should be read as an affine triangular transport-map density model, not as the full nonlinear adaptive map of [8]. This restriction is intentional: the RQ5 notebooks repeatedly evaluate many intervention settings, random seeds, source subsets, and $L$ values, so stability and transparent failure modes are more important than maximizing density-model flexibility. To recover part of the nonlinear source geometry while keeping the density model simple, the source block is lifted before density estimation. For a single source $x$, the lifted block is $(x,x^2,x^3)$; for two sources $(x,y)$, it is $(x,y,xy,x^2,y^2)$. The cross term $xy$ is essential for the alpha-controlled experiment because the injected nonlinear mechanism contains $\sin(Q_2^nQ_3^n)$.

Given source samples $\mathbf{x}^{(m)}$ and target samples $\mathbf{y}^{(m)}$, mutual information is estimated from transport-map log densities as

$$
\widehat{I}^{\mathrm{tm}}(X;Y)
=
\frac{1}{M}\sum_{m=1}^{M}
\left[
\log \widehat{p}_{\mathrm{tm}}(\mathbf{x}^{(m)},\mathbf{y}^{(m)})
-
\log \widehat{p}_{\mathrm{tm}}(\mathbf{x}^{(m)})
-
\log \widehat{p}_{\mathrm{tm}}(\mathbf{y}^{(m)})
\right]
-
\widehat{b}_{M,d},
\tag{K.4}
...
where $\widehat{b}_{M,d}$ is the finite-sample log-determinant correction used for the affine Gaussian covariance estimates. The notebook-level two-source synergy is then computed as

$$
\widehat{\mathrm{Syn}}^{\mathrm{tm}}(X_1,X_2;Y)
=
\widehat{I}^{\mathrm{tm}}\!\bigl(\phi_{12}(X_1,X_2);Y\bigr)
-
\widehat{I}^{\mathrm{tm}}\!\bigl(\phi_{1}(X_1);Y\bigr)
-
\widehat{I}^{\mathrm{tm}}\!\bigl(\phi_{2}(X_2);Y\bigr),
\tag{K.5}
$$

with nonnegative clipping applied to each individual $EI$ estimate before subtraction. This convention preserves the intended joint-minus-single semantics: positive values indicate information available only after the sources are observed jointly, while values near zero indicate that the target can already be explained by single-source terms. Small negative values are therefore interpreted as finite-sample or model-mismatch artifacts rather than as evidence for a separate negative-synergy mechanism.

This method directly supports the current TM-only notebook. In the alpha sweep, the intervention distribution is $Q_2^n,Q_3^n\sim\mathrm{Uniform}[-L/2,L/2]$ and the target is generated by

$$
Q_1^{n+1}
=
\alpha\sin(Q_2^nQ_3^n)
+
(1-\alpha)Q_2^n
+
\varepsilon,
...
At $\alpha=0$, the mechanism is single-source and the expected $\widehat{\mathrm{Syn}}^{\mathrm{tm}}$ should be close to zero. As $\alpha$ increases, the joint nonlinear term becomes dominant, so the estimated synergy ratio $\widehat{\mathrm{Syn}}^{\mathrm{tm}}/\widehat{EI}^{\mathrm{tm}}$ should rise while the single-source contribution from $Q_2^n$ declines. The additional $L$ sweep is not merely a scale check: changing $L$ changes how much of the oscillatory $\sin(Q_2^nQ_3^n)$ surface is covered by the intervention distribution, and therefore tests the interaction between intervention support, nonlinear geometry, and the low-order lifted affine map. In this sense, the transport-map estimator is used as a distribution-level readout of the same structural question addressed by the Jacobian--Covariance formula: whether a target response is explainable by separable source effects or requires a genuinely joint source direction.

### K.2 连续密度估计器 benchmark 补充图与说明
...
## 附录 M. 空气污染实验中的 MLP 构建与训练细节

本附录补充正文第 6.7 节空气污染实验中预测器的构建细节。正文只报告 `transport map` 因果图和主要解释，本节说明这些图所依赖的多站点状态转移模型如何从小时级观测数据中训练出来。这里的核心作用不是提出一个新的空气质量预测模型，而是获得一个足够可用、可微、并且对所有站点联合建模的状态转移函数，然后在该函数上估计 `EI^{\mathrm{tm}}` 与 `Syn^{\mathrm{tm}}`。

### M.1 样本构造与标准化

当前代表性实验使用杭州 `12h` 预测设置。原始数据保持 `1h` 分辨率，样本模式为 `one_step`，即每个样本只取当前时刻的全站点联合快照作为输入，不额外展开历史窗口。当前缓存的杭州运行包含 `12` 个站点、每站 `10` 个输入变量，因此单个输入样本的形状为 `12 x 10`，展平后的有效输入维度为 `120`。输入变量顺序为两个污染物变量 `O3`、`PM2.5` 加上八个气象变量 `t2m`、`d2m`、`sp`、`tp`、`blh`、`msdwswrf`、`u100`、`v100`。

目标变量只包含 `O3` 与 `PM2.5`，并在所有站点上联合预测。对 horizon $h=12$，训练样本可概括为

$$
\mathbf{x}_t
=
\bigl(\mathbf{s}_{1,t},\dots,\mathbf{s}_{N,t}\bigr),
\qquad
\mathbf{y}_{t+h}
=
\bigl(\mathrm{O3}_{1,t+h},\mathrm{PM2.5}_{1,t+h},\dots,\mathrm{O3}_{N,t+h},\mathrm{PM2.5}_{N,t+h}\bigr),
\tag{M.1}
$$

其中 $N=12$，$\mathbf{s}_{i,t}$ 是第 $i$ 个站点在当前时刻的 `10` 维状态向量。因此模型输出维度为 `24`。这一定义保证预测器不是逐站点独立模型，而是一个从“全站点当前联合状态”到“全站点未来污染物联合状态”的映射。

数据按目标时刻划分训练、验证与测试集：未来时刻不晚于 `2021-12-31 23:00:00` 的样本进入训练集，`2022` 年进入验证集，`2023` 年进入测试集。当前杭州 `12h` 运行得到 `52596` 个训练样本、`8760` 个验证样本和 `8760` 个测试样本。所有输入变量与目标变量先用训练期统计量做标准化；均值和标准差按变量在训练期全部站点与全部时刻上估计，然后同一变量在验证、测试和反标准化时复用同一组统计量。

### M.2 JointStationMLP 的结构

空气污染实验使用 `JointStationMLP` 的 `resmlp` 变体。其输入首先被展平成一个长度为 `120` 的向量，再送入一个共享 trunk；每个预测 horizon 使用一个独立线性 head。当前 `12h` 实验只有一个 horizon，因此只有一个 `12h` head。

具体结构为：

1. 输入投影：`Linear(120, 96)`，随后接 `SiLU` 激活。
2. 残差 trunk：连续 `3` 个 residual MLP block。
3. 每个 residual block 内部为 `LayerNorm(96) -> Linear(96, 96) -> SiLU -> Dropout(0.05) -> Linear(96, 96) -> Dropout(0.05)`，最后与 block 输入相加。
4. trunk 末尾再接一个 `LayerNorm(96)`。
5. 输出 head：`Linear(96, 24)`，直接预测所有站点的 `O3` 与 `PM2.5` 标准化值。
...
=
g_h\!\left(f_{\theta}(\mathbf{x}_t)\right),
\tag{M.2}
$$

其中 $f_{\theta}$ 是共享 ResMLP trunk，$g_h$ 是 horizon-specific 线性 head。这里选择联合 MLP 而不是逐站点模型，是因为后续因果图需要比较跨站 `O3 -> O3`、`PM2.5 -> O3` 与 `O3 + PM2.5 -> O3` 的作用；若先把每个站点分开训练，跨站输入的相互作用会在模型结构上被人为削弱。

### M.3 训练目标、早停与基线

训练使用 Adam 优化器，当前杭州 `12h` 配置为学习率 `5e-4`、weight decay `1e-5`、batch size `64`。每个 epoch 内对训练样本做随机排列，并按 mini-batch 更新。损失函数是各 horizon 的均方误差之和：
...
当前空气污染搜索每次只固定一个 horizon，所以在杭州 `12h` 实验里，式 (M.3) 实际就是 `12h` 目标上的多输出 MSE。训练循环最多允许 `60` 个 epoch，并以验证集 MSE 做模型选择；若连续 `8` 个 epoch 未刷新最佳验证损失，则触发早停。当前缓存记录的杭州 `12h` 运行选择第 `6` 个 epoch 的参数，最佳验证损失为 `0.3869`，训练历史记录到第 `14` 个 epoch。

评估时，模型输出先从标准化尺度反变换回原始量纲，再计算 `RMSE`、`MAE` 与全量展平相关系数。基线模型为 persistence baseline：它直接把当前输入快照中的 `O3` 与 `PM2.5` 作为未来所有 horizon 的预测值。该基线与 `JointStationMLP` 使用同一测试集和同一反标准化流程，因此可以作为“模型是否学到超出持续性假设的信息”的最低门槛。

当前杭州 `12h` 测试结果为：persistence baseline 的总体 `RMSE=50.565`、`MAE=33.119`、相关系数 `0.203`；`JointStationMLP` 的总体 `RMSE=26.006`、`MAE=18.195`、相关系数 `0.764`。这说明后续因果图并不是在一个退化预测器上解释噪声，而是在一个明显优于持续性基线的多站点状态转移模型上做分布层面的作用分解。

### M.4 与 transport-map 因果图的衔接

训练后的 `JointStationMLP` 在因果图阶段被视为固定的状态转移函数。对每一个源站点和目标站点组合，实验在当前状态空间中构造干预样本，并用 transport-map 密度估计口径分别计算单变量 `O3 -> O3`、单变量 `PM2.5 -> O3`，以及同一源站点 `O3 + PM2.5 -> O3` 的联合项。协同项再由联合项减去两个单变量项得到。

因此，MLP 在本实验中的角色可以分成两层。第一层是预测层：它把全部站点当前污染物和气象状态映射到 `12h` 后全部站点污染物状态，并通过验证集选择参数。第二层是机制读出层：在模型参数固定后，只改变输入干预分布和目标站点索引，读取不同源变量对子目标 `O3` 的 `EI^{\mathrm{tm}}` 与 `Syn^{\mathrm{tm}}`。正文第 6.7 节报告的三面板图，正是基于这个固定预测器导出的 top-10 非 self 因果边。

需要注意，当前图目录中的 `seed42` 指 transport-map 干预采样的随机种子，而不是 MLP 训练种子。MLP 训练配置本身使用固定随机种子 `0`；因此，模型训练随机性与后续 Monte Carlo 干预采样随机性在实现上是分开的。
