...
实验以 Lausanne-33 count 数据派生的 83 脑区结构连接矩阵近似复现 Mediano et al. (2025) 的 DMF Fig. 6 设置，并扫描
...
需要注意，\(\Phi^{EID}\) 峰值与 full-pair \(\Phi^R\) 峰值相差一个扫描步长（\(0.1\)）。在当前离散网格和近似结构连接矩阵下，这支持二者共同指向同一宽转变区，但不足以声称两种指标识别了完全相同的临界点。
...
- 当前使用 Lausanne-33 count 派生的 83 区近似矩阵，而不是原论文未随数据包公开的 HCP Lausanne-83 精确结构连接矩阵，因此只能视为 Fig. 6 的近似复现。
...
相关文献为 Mediano et al. (2025), *Toward a unified taxonomy of information dynamics via Integrated Information Decomposition*，以及 Yang, Wang, and Zhang (2026), *Partial Effective Information Decomposition for Synergistic Causality*。前者说明 \(\Phi^R\) 在 DMF 临界区附近的经验行为，后者给出最大熵干预下 PEID 协同的机制解释。
