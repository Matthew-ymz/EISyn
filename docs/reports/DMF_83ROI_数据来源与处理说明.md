...
为避免把当前实验与 HCP 扩散 MRI 结构连接混淆，下图直接展示实际进入 DMF 的矩阵 $\mathbf{C}$：它是 F-TRACT probability 条目中的 CCEP `count` 经删除 `Unknown`、全局缩放和对角清零后得到的 83 ROI 有向代理耦合矩阵，**不是 HCP dMRI 纤维矩阵**。

![F-TRACT CCEP count 派生的 83 ROI DMF 代理耦合矩阵](../../fig/dmf_83roi_proxy_connectivity_matrix.png)

**图 1｜83 ROI DMF 代理耦合矩阵。** 行和列保留 F-TRACT 原始 ROI 顺序；按官方元数据，行为 stimulated ROI，列为 recorded ROI。彩色边条依次标出左皮层、左皮层下、右皮层、右皮层下和脑干。为让较弱的非零连接仍可见，热图仅在显示映射中使用 $\gamma=0.45$ 的幂次归一化；色条标注仍是原始 $C_{jk}$ 权重。

矩阵中左、右半球各自的皮层内块最为突出，跨半球皮层块以更弱、更稀疏的连接为主；皮层下和脑干相关行列也相对稀疏。对角线为清零后的自连接；主对角两侧强度近似但不完全镜像，与该矩阵保留有向性的处理口径一致。
