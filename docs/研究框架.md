...
<p align="center">
  ...
  这张图说明，`TM` 口径下的跨站 `O3 -> O3` 依赖并不是均匀铺满全市，而是集中在少数站点对之间。当前最强显示跨站边为 `3265A \to 3268A`，平均强度约 `0.086`；较显著的连边还集中出现在 `3268A`、`3270A`、`3271A`、`3272A`、`3273A` 与 `3544A` 这一簇相邻站点之间。相比之下，`1h` 全量 pairwise 边里虽然也有 `67` 条负值边，但它们的绝对值中位数只有 `0.000202`，最大也只有 `0.000244`，明显小于图中主要正边的量级。因此，对当前结果更合理的解读是：`TM` 因果图已经稳定恢复出一组空间上可定位、强度上可排序的主导跨站 `O3` 依赖，而零附近的小负值更接近数值估计伪影。
